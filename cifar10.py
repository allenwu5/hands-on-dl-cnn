...
for fn in train_fns:
	raw_data = unpickle(dataset_path + fn)
	if fn == train_fns[0]:
		img_px_values = raw_data[b'data']
		img_lab = raw_data[b'labels']
	else:
		img_px_values = numpy.vstack((img_px_values, raw_data[b'data']))
		img_lab = numpy.hstack((img_lab, raw_data[b'labels']))
...
Y_test = np_utils.to_categorical(numpy.array(raw_data[b'labels']), classes)
